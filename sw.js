...
/* Bump this string on every launcher change.
 *
 * The activate handler deletes every cache whose name is not the current one,
 * so renaming it is what actually evicts an old splash from a phone that has
 * already installed the app. The fetch handler is network-first, which covers
 * an ordinary browser visit, but an installed PWA can serve the cached shell
 * before the network is consulted — the school saw a build-42 splash long
 * after newer files existed. A new cache name removes that possibility. */
var CACHE = 'fsms-shell-v45';
